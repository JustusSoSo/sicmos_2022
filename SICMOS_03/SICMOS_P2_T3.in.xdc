...
# Breakdwon Voltage
    ...
    ################ contact ################
    models srh fermidirac conmob fldmob auger analytic optr bgn temperature=300 CVT print
    impact selb aniso sic4h0001 e.side
    method itlimit=70 climit=1e-20 ix.tol=1e-38 ir.tol=1e-38 px.tol=1.e-20 pr.tol=1.e-43 cx.tol=1.e-20 cr.tol=1.e-35
    # output flowlines e.mobility h.mobility
    solve init previous
    log outfile=./LOG/B_BV_SCH_w=$'sch_barrier'.log
    solve prev vfinal=0.001
    solve prev vfinal=0.01
    solve prev vfinal=0.1
    solve prev vfinal=1
    solve prev vfinal=5 vstep=1 name=drain
    solve prev vfinal=20 vstep=5 name=drain
    solve prev vfinal=50 vstep=10 name=drain
    ...
    log close
    extract name="BV_sch_barrier=$'sch_barrier'" \
    x.val from curve(abs(v."drain") ,abs(i."drain")) where y.val=1e-22 \
    datafile="data04_BV.dat"
    extract name="Vmax_sch_barrier=$'sch_barrier'" \
    x.val from curve(v."drain",i."drain") where y.val = max(i."drain") \
    datafile="data04_BV_Vmax.dat"
    quit
# Breakdwon Voltage
...
# log outfile=./LOG/B_Ids-Vds_BD_
# SCH_w=$'sch_barrier'.log
# extract name="Von_
# sch_barrier=$'sch_barrier'"