...
# Ids - Vds diode
    ...
    models srh cvt fermidirac conmob fldmob bgn print
    models print
    ################ SETTING ################
    method climit=1e-4 maxtrap=15
    solve init
    solve name=gate vstep=5 vfinal=20
    set VDS_step    =0.5
    set VDS_final   =25
    ################ MODIFY ################
    # log outfile=./LOG/B_Ids-Vds_MOS=$'SCH_width'.log
    # log outfile=./LOG/B_Ids-Vds_MOS=$'sch_barrier'.log
    # log outfile=./LOG/B_Ids-Vds_MOS=$'P_well_t'.log
    log outfile=./LOG/B_Ids-Vds_MOS=$'P_well'.log
    ################ MODIFY ################
    solve name=drain \
    vstep=$VDS_step vfinal=$VDS_final
    # Ron（Vgs=20V,Vds=2V) Vds = 0-2V
    set tmp_x1=0.5
    set tmp_x2=5.0
    ################ MODIFY ################
    extract name="Ronsp20V=$'P_well'" \
    min(curve(v."drain", v."drain"/i."drain", x.min=$tmp_x1 x.max=$tmp_x2)) \
    * 1e3 * $DEVICE_A \
    datafile="data02_ronsp.dat"
    ################ MODIFY ################
    quit
# Ids - Vds diode
